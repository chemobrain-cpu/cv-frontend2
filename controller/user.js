...
module.exports.initiatePlan = async (req, res) => {
   const { email, amount, plan,subscriptionType } = req.body; // Extract data from the request body
   ...
   try {
      // Make a POST request to the Paystack API for transaction initialization
      const response = await axios.post('https://api.paystack.co/transaction/initialize', params, {
         //${PAYSTACK_SECRET_KEY}
         headers: {
            'Authorization': `Bearer sk_test_e4c0b2a1af89770feb185eb2ab17044aab05b92f`,
            'Content-Type': 'application/json'
         }
      });

      if (response.status === 200) {
         // check if it as successful and then do something to the current user.. 
         ...
         userExist.paymentPlan = subscriptionType
         // Save the updated user info to the database
         let updatedUser = await userExist.save();
         if (!updatedUser) {
